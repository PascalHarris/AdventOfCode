...
basinSize[start_] := 
 Module[{visited = {}, queue = {start}}, 
  While[queue != {}, pos = First[queue];
   queue = Rest[queue];
   If[! MemberQ[visited, pos] && grid[[pos[[1]], pos[[2]]]] != 9, 
    AppendTo[visited, pos];
    queue = Join[queue, neighbors[pos]]]];
  Length[visited]]

Times @@ TakeLargest[basinSize /@ lowPoints, 3]
